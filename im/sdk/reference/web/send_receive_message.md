...
2. **发送方** 构建富媒体消息，指定 NOS 存储场景。
    ...
    **图片消息示例代码：**
    ...
    // 而 uniapp/小程序/RN 等只允许输入为 filePath 临时路径字符串
    // uni.chooseImage, wx.chooseImage 等等 API 得到临时上传路径, 这里简单演示一下 wx 的
    wx.chooseMedia({
      count: 1,
      mediaType: ['image', 'video'],
      fail(err) {
        console.error('chooseMediaFail', err)
      },
      success: async (response) => {
        // 选择后返回 {errMsg: "chooseMedia:ok", failedCount: 0, type: "image", tempFiles: Array(1)}
        const tempFiles = response.tempFiles
        const tempFile = tempFiles[0]
        if (!tempFile) {
          console.error('chooseMediaEmpty')
          return
        }
        // 选择了返回 {tempFilePath: "http://tmp/DbreCjC4p5DV337cb3a5254fc820c3b95827b628472d", size: 6030592, fileType: "image", width: 3000, height: 5500}
        const tempFilePath = tempFile.tempFilePath
        try {
          nim.V2NIMMessageCreator.createImageMessage(tempFilePath)
        } catch (err) {
    ...
    **语音消息示例代码：**
    ...
    wx.chooseMedia({
      ...
      success: async (response) => {
        // 选择后的真实返回 {errMsg: "chooseMedia:ok", failedCount: 0, type: "audio", tempFiles: Array(1)}
        const tempFiles = response.tempFiles
        const tempFile = tempFiles[0]
        if (!tempFile) {
          console.error('chooseMediaEmpty')
        ...
        // 选择后的真实 tempFile 返回 {tempFilePath: "http://tmp/DbreCjC4p5DV337cb3a5254fc820c3b95827b628472d", size: 6030592, fileType: "audio", duration: 6.198345, width: 0, height: 0}
        const tempFilePath = tempFile.tempFilePath
        try {
          nim.V2NIMMessageCreator.createAudioMessage(tempFilePath)
        } catch (err) {
          console.log('创建失败', err.toString())
        }
      }
    ...
    **视频消息示例代码：**
    ...
    wx.chooseMedia({
      ...
      success: async (response) => {
        // 选择后的真实返回 {errMsg: "chooseMedia:ok", failedCount: 0, type: "video", tempFiles: Array(1)}
        const tempFiles = response.tempFiles
        const tempFile = tempFiles[0]
        if (!tempFile) {
          console.error('chooseMediaEmpty')
        ...
        // 选择后的真实 tempFile 返回 {tempFilePath: "http://tmp/DbreCjC4p5DV337cb3a5254fc820c3b95827b628472d", size: 6030592, fileType: "video", duration: 6.198345, width: 0, height: 0}
        const tempFilePath = tempFile.tempFilePath
        try {
          nim.V2NIMMessageCreator.createVideoMessage(tempFilePath)
        } catch (err) {
          console.log('创建失败', err.toString())
        }
      }
    ...
    **文件消息示例代码：**
    ...
    wx.chooseMedia({
      ...
      success: async (response) => {
        ...
        const tempFilePath = tempFile.tempFilePath
        try {
          nim.V2NIMMessageCreator.createFileMessage(tempFilePath)
        } catch (err) {
          console.log('创建失败', err.toString())
        }
      }
