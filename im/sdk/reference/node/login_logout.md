...
- **添加监听**：[`on("EventName")`](https://doc.yunxin.163.com/messaging2/client-apis/TQ5NTUwNzQ?platform=client#on)
...
- **移除监听**：[`off("EventName")`](https://doc.yunxin.163.com/messaging2/client-apis/TQ5NTUwNzQ?platform=client#off)
...
- **添加监听**：[`on("EventName")`](https://doc.yunxin.163.com/messaging2/client-apis/TQ5NTUwNzQ?platform=client#on)
    ...
    ```TypeScript
    v2.loginService.on("onConnectStatus", function (status: V2NIMConnectStatus) {})
    v2.loginService.on("onDisconnected", function (error: V2NIMError) {})
    v2.loginService.on("onConnectFailed", function (error: V2NIMError) {})
    v2.loginService.on("onDataSync", function (type: V2NIMDataSyncType, state:    V2NIMDataSyncState, error?: V2NIMError) {})
    ```
...
```TypeScript
try {
    await v2.loginService.login("ACCOUNT_ID", "", {
    "forceMode": false,
...
API | 说明
--- | ---
[`on("EventName")`](https://doc.yunxin.163.com/messaging2/client-apis/TQ5NTUwNzQ?platform=client#on) | 注册登录相关监听器
[`off("EventName")`](https://doc.yunxin.163.com/messaging2/client-apis/TQ5NTUwNzQ?platform=client#off) | 取消注册登录相关监听器
